...
diabetes$m <- simLogisticMissingness0(data    = diabetes,
                                      pm      = 0.25,
                                      preds   = c("bmi", "tc"),
                                      type    = "high",
                                      stdData = TRUE)$r
...
<<echo = FALSE, cache = TRUE, warning = FALSE>>=
miceM <- mice(data      = dat2[ , c("bmi", "glu")],
              m         = 1,
              maxit     = 1,
              method    = "mean",
              printFlag = FALSE)
...
<<echo = FALSE, cache = TRUE, warning = FALSE>>=
miceD <- mice(data      = dat2[ , c("bmi", "glu")],
              m         = 1,
              maxit     = 1,
              method    = "norm.predict",
              printFlag = FALSE)
...
grid.arrange(
p0 + geom_line(aes(group = id), alpha = 0.3) + geom_point(),
...
<<echo = FALSE, cache = TRUE, fig.asp = 0.5>>=
grid.arrange(
p1 + geom_line(data = dat3, aes(group = id, color = m2), alpha = 0.3) +
