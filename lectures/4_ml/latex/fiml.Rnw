...
\begin{frame}[allowframebreaks]{Satisfying the MAR Assumption}
  ...
  included them in the analysis model will change the model's meaning.
  \vc
  \begin{itemize}
...
\begin{frame}{Saturated Correlates Technique}
  ...
  \item Satisfy the MAR assumption by incorporating MAR predictors into the 
    analysis model.
    \vc
  \item Not affecting the meaning of the substantively interesting parameters.
  \end{enumerate}
  \vb
  Incorporate the MAR predictors via a fully-saturated covariance structure:
  \vc
  \begin{enumerate}
  \item Allow all MAR predictors to covary with all other MAR predictors.
    \vc
  \item Allow all MAR predictors to covary with all observed variables in the
