...
\begin{frame}[fragile]{Example}
  ...
  diabetes1 <- diabetes2 <-
      readRDS(paste0(dataDir, "diabetes_norm.rds"))
...
\begin{frame}[fragile]{Example}
  ...
  <<>>=
  s1 <- lm(glu ~ bmi + bp, data = diabetes1) %>% summary()
  s2 <- lm(glu ~ bmi + bp, data = diabetes2) %>% summary()
...
\begin{frame}[fragile]{Example}
  ...
  imputed[m, "bp"]  <- mean(imputed$bp, na.rm = TRUE)
...
\begin{frame}[fragile]{Example}
  ...
  <<>>=
  s1 <- lm(glu ~ bmi + bp, data = diabetes1) %>% summary()
  s2 <- lm(glu ~ bmi + bp, data = imputed) %>% summary()
...
\begin{frame}[fragile]{Example}
  ...
  <<>>=
  fit1 <- lm(glu ~ bmi + bp, data = diabetes1)
