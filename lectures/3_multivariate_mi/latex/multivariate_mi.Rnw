...
\begin{frame}{Multivariate Bayesian Regression}
  ...
  following model:
  \begin{align*}
    \sigma^2 &\sim \text{Inv-}\chi^2 \left(N - P, MSE \right)\\[8pt]
  ...
  \begin{align*}
    \Sigma^{(i)} &\sim \text{Inv-W} \left(N - 1, (N - 1) \Sigma^{(i - 1)} \right)\\[8pt]
...
\begin{frame}[fragile, allowframebreaks]{Example: Data Visualization}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      Use \code{ggmice::plot\_pattern()} to visualize the response patterns.
