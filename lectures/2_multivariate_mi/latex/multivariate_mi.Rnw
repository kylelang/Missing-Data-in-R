...
\begin{frame}[fragile]{Example: Data Generation}
  ...
  Next, we impose some missing values on the simluated data.
...
\begin{frame}{FCS for Mixed Data Types}
  ...
  FCS imputation can easily accomodate incomplete data that contain both 
  continous and categorical/non-normal variables.
  \vc
  \begin{itemize}
  ...
  \end{itemize}
  \vb
  The FCS framework can essentially accomodate any data for which you can define 
